...
<head>
  ...
  <!-- build: v-center-card-fixflip-2025-08-13 -->
  <style>
    ...
    .card{
      ...
      max-width:720px;margin:0 auto; /* 卡片左右置中 */
...
<body>
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    // 3) 偵測「反向參數」並自動翻轉：
    //    例： ?2025-08-13=訂單時間  →  轉成 { 訂單時間: 2025-08-13 }
    const fixed = {...raw};
    for (const [k,v] of Object.entries(raw)) {
      if (!allAliasWords.has(k) && allAliasWords.has(v)) {
        // value 看起來像欄位名稱、key 像值 → 翻轉
        fixed[v] = k;
        // 若原本也有正確鍵就以正確鍵為主，不覆蓋
      }
    }

    // 4) 依順序產生英文鍵（EmailJS 用）與畫面表格
    ...
    order.forEach(key=>{
      ...
      rows += `<tr><th>${labels[key]}</th><td>${value || "<span class='muted'>（空）</span>"}</td></tr>`;
    });
    tbody.innerHTML = rows;

    // 5) 靜默送 Make（把所有原始欄位 + 一份 __emailjs__）
